...
def multihead_attention_counter_hook(multihead_attention_module, input, output):
    ...
    if len(input) == 4:  # modify by ryan
        q, k, v, _ = input
    else:
        raise 'ERROR! ONLY SUPPORT MMCV SELF-ATTEN'
    ...
    if hasattr(multihead_attention_module, 'embed_dim'):
        assert qdim == multihead_attention_module.embed_dim
    else:
        assert qdim == multihead_attention_module.embed_dims
    ...
    if hasattr(multihead_attention_module, 'in_proj_bias'):
        if multihead_attention_module.in_proj_bias is not None:
            flops += (qlen + klen + vlen) * qdim
    elif hasattr(multihead_attention_module.attn, 'in_proj_bias'):
        if multihead_attention_module.attn.in_proj_bias is not None:
            flops += (qlen + klen + vlen) * qdim
...
def Detr3dMul_flops_counter_hook(Detr3dMul, input, output):
    x, y = input
    b, d, num_queries, cam_num, num_points, levels = x.shape
    flops = b*d*num_queries*cam_num*num_points*levels
    Detr3dMul.__flops__ += int(flops)
...
from projects.mmdet3d_plugin.models.utils.detr3d_transformer import Detr3dMul
...
MODULES_MAPPING = {
    ...
    NonDynamicallyQuantizableLinear: linear_flops_counter_hook,
    ...
    Detr3dMul: Detr3dMul_flops_counter_hook,
}
